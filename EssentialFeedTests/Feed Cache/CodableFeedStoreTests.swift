...
//  EssentialFeedTests
//
//  Created by João Noivo on 13/02/2023.
//
...
class CodableFeedStoreTests: XCTestCase {
  ...
  func test_retrieveAfterInsertingToEmptyache_deliversInsertedValues() {
    let sut = makeSUT()
    ...
    sut.insert(feed, timestamp: timestamp) { insertionError in
      ...
      sut.retrieve { firstResult in
        sut.retrieve { secondResult in
          ...
          default:
            XCTFail("Expected retrieving twice from non empty cache to deliver same found result with feed \(feed) and timesstamp \(timestamp). got \(firstResult) and \(secondResult) instead")
          }
          exp.fulfill()
        }
      }
